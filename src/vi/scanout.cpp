...
VideoField Bus::scan_video(VideoScanMode mode) const {
    settle_system();
    ...
    const auto render = [&](s32 begin, s32 end, unsigned) {
    ...
    const bool parallel = mode == VideoScanMode::Parallel && (last_x - first_x) * (last_y - first_y) >= 65536;
    tasks::parallel_ranges(first_y, last_y, parallel, render);
    return output;
}
