...
u64 Bus::next_event() const {
    ...
    include(pi_dma_pending_ && pi_dma_progress_counter_ != 0, pi_dma_progress_counter_);
    ...
    if (rtc && rtc->running())
        include(true, rtc->next_tick());
    for (const auto& pak : transfer_paks) {
        if (pak.clock_running())
            include(true, pak.next_tick());
    }
    ...
    if (video_output_ || audio_sample_output ||
        (ai_fifo_count_ != 0 && (ai_lengths_[0] == 0 || (ai_[2] & 1U) != 0))) {
