...
TEST(rsp_dma_completion_preserves_addresses_staged_while_busy) {
    for (const bool to_sp : {false, true}) {
        for (const u32 bank : {0U, 0x1000U}) {
            ...
            write_register(*system, length_register, 15);
            system->rsp.tick(1);
            ...
            system->rsp.tick(1);
            check_copy(*system, bank, to_sp, 0, 0x200, 1);
            check_copy(*system, bank, to_sp, 8, 0x208, 9);
            CHECK_EQ(read_register(*system, 0x00), bank + 16);
            CHECK_EQ(read_register(*system, 0x04), 0x210U);
    ...
    for (const bool to_sp : {false, true}) {
        for (const bool single : {false, true}) {
            ...
            const std::array<u32, 12> program = {
                0x240100c0, // ADDIU at, zero, 0xc0
                0x40810000, // MTC0 at, SP_MEM_ADDR
                0x240102c0, // ADDIU at, zero, 0x2c0
            ...
            write_register(*system, 0x04, 0x200);
            write_register(*system, length_register, 95); // Twelve RCP cycles, past the first BREAK.
            ...
            CHECK_EQ(read_register(*system, 0x18), 0U);
            CHECK_EQ(read_register(*system, 0x00), 0x60U);
            CHECK_EQ(read_register(*system, 0x04), 0x260U);
            CHECK_EQ(read_register(*system, 0x10) & 3U, 3U);

            write_register(*system, 0x10, 5);
            advance_cpu(*system, 9, single);
            CHECK_EQ(system->bus.read(0x04000084, 4), 0x60U);
            CHECK_EQ(system->rsp.pc, 44U);
            CHECK_EQ(read_register(*system, 0x10) & 3U, 0U);
            CHECK_EQ(read_register(*system, 0x18), 1U);
            CHECK_EQ(read_register(*system, 0x00), 0xc0U);
            CHECK_EQ(read_register(*system, 0x04), 0x2c0U);
            for (u32 byte = 0; byte < 8; ++byte) {
                CHECK_EQ(to_sp ? system->rsp.memory[0xc0 + byte] : system->bus.read_ram_byte(0x2c0 + byte),
                         0U);
            }
            // MFC0's load interlock moves the length write to cycle 18. Its row completes at 19.
            advance_cpu(*system, 2, single);
            CHECK_EQ(system->bus.read(0x04100010, 4), 19U);
            CHECK_EQ(system->rsp.pc, 48U);
            check_copy(*system, 0, to_sp, 0xc0, 0x2c0, 0xc1);
            CHECK_EQ(read_register(*system, 0x18), 0U);
            CHECK_EQ(read_register(*system, 0x00), 0xc8U);
            CHECK_EQ(read_register(*system, 0x04), 0x2c8U);
            CHECK_EQ(read_register(*system, 0x10) & 3U, 3U);
        }
    }
}
